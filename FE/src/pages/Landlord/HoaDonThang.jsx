...
function LandlordHoaDonThang() {
  const [isLoading, setIsLoading] = useState(false);
  const [hdThang, setHdThang] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const { data } = await axiosInstance.get("/landlord/hoadonthang");
      setHdThang(data.data);
    } catch (error) {
      console.error("Lỗi lấy hóa đơn:", error);
      toast.error("Không thể lấy danh sách hóa đơn tháng!");
    }
  };
  ...
  const handleDelete = async (value) => {
    try {
      await axiosInstance.delete(`/landlord/hoadonthang/delete/${value._id}`);
...
export default LandlordHoaDonThang;
