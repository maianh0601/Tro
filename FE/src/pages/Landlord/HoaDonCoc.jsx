...
function LandlordHoaDonCoc() {
  const formatDate = (dateString) =>
    new Date(dateString).toLocaleDateString("vi-VN");
  const { isOpen } = useSelector((state) => state.ModalForm);
  const [data, setData] = useState("");
  const [hdCoc, setHdCoc] = useState([]);
  const dispatch = useDispatch();

  const fetchData = async () => {
    try {
      const { data } = await axiosInstance.get("/landlord/hoadoncoc");
      setHdCoc(data.data);
    } catch (error) {
      console.error("Lỗi lấy hóa đơn:", error);
      toast.error("Không thể lấy danh sách hóa đơn cọc!");
    }
  };
  ...
  const headers = [
    ...
    { label: "Mã hóa đơn", key: "ma_don_hang" },
  ...
  const handleDelete = async (value) => {
    try {
      await axiosInstance.delete(`/landlord/hoadoncoc/delete/${value._id}`);
  ...
  const handleOpenModalEdit = async (value) => {
    ...
    const res = await axiosInstance.get(`/landlord/hoadoncoc/detail/${value._id}`);
    setData(res.data);
  };
  ...
  const handleUpdateStatus = async (newStatus) => {
    if (!data?._id) return;
    try {
      await axiosInstance.patch(`/landlord/hoadoncoc/update-status/${data._id}`, {
      ...
      const updated = {
        ...
        trang_thai: newStatus,
        ngay_chuyen_khoan:
          newStatus === "đã thanh toán" ? new Date().toISOString() : data.ngay_chuyen_khoan,
      };
      setData(updated);

      setDsHienThi((prev) =>
        prev.map((item) => (item._id === updated._id ? { ...item, trang_thai: updated.trang_thai } : item))
      );
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-xl">
            ...
            <div className="ml-3 flex gap-3">
              {data.trang_thai !== "đã thanh toán" ? (
                ...
                <button
                  onClick={() => handleUpdateStatus("chưa thanh toán")}
...
export default LandlordHoaDonCoc;
